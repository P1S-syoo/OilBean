...
namespace Game.Surface {
    ...
    public class OrbitCameraDriver : MonoBehaviour {
        ...
        [SerializeField] float sensX;   // 카메라 수평 감도 — 기본값은 수면위설정.마우스좌우감도
        [SerializeField] float sensY;   // 카메라 수직 감도 — 기본값은 수면위설정.마우스상하감도
        ...
        void OnEnable() {
            look?.Enable();
            // 커서는 잠그지 않는다 — CustomCursorFx가 OS 커서를 숨기고 홀로그램 포인터를 렌더
